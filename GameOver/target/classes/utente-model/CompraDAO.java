...
public class CompraDAO {
    ...
    public CompraBean doRetrieveByUsername(String codice) throws SQLException {
        ...
        try {
            ...
            while(rs.next()) {
               ...
               b.setIdVideogioco(rs.getString("id"));
            }
            rs.close();
        }finally {
            try {
    ...
    public boolean doDelete(CompraBean utente) throws SQLException {
        String query="delete from "+ CompraBean.TABLE_NAME+" where id=?";
        Connection con=null;
        PreparedStatement ps=null;
        ...
        try {
            con= DriverManagerConnectionPool.getConnection();
            ps=con.prepareStatement(query);
            ps.setString(1, utente.getIdVideogioco());
            ps.execute();
        }finally {
            try {
                if(ps!=null) ps.close();
